...
interface TreatmentHistoryProps {
  ...
  teeth: Tooth[]; // Not directly used for names anymore, but kept for potential future use
  onEditTreatment: (treatment: Treatment) => void;
  onDeleteTreatment: (treatmentId: string) => void;
}

const TreatmentHistory: React.FC<TreatmentHistoryProps> = ({ treatments, teeth, onEditTreatment, onDeleteTreatment }) => {
  ...
  return (
    <div className="space-y-4">
      {/* Title already present in App.tsx */}
        ...
        <table className="min-w-full bg-white">
          ...
          <tbody className="divide-y divide-gray-200">
            {treatments.sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(treatment => (
              <tr key={treatment.id} className="hover:bg-gray-50/70 transition-colors duration-150">
                ...
                <td className="px-5 py-4 whitespace-nowrap text-sm space-x-3">
                  <button 
                    ...
                    className="text-primary hover:text-primary-dark font-medium hover:underline focus:outline-none focus:underline"
                  ...
                  <button 
                    onClick={() => {
                        if(window.confirm('Apakah Anda yakin ingin menghapus riwayat perawatan ini? Tindakan ini tidak dapat diurungkan.')) {
                            onDeleteTreatment(treatment.id)
                        }
                    }}
                    className="text-red-600 hover:text-red-800 font-medium hover:underline focus:outline-none focus:underline"
